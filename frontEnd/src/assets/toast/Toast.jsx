...
export const useToast = () => {
    ...
    if(!context) {
        throw new Error("Pour utiliser le toast, vous devez envelopper votre application avec ToastWraper")
    }
    ...
    return (
        <ToastContext.Provider value={{
            toast
                ...
                {show&&
                <div className="fixed top-8 left-1/2 -translate-x-1/2 z-999999">
                    <div className="bg-brand-600 text-white rounded-md p-2 flex items-center gap-2">
                        
                        <div className={`p-1 w-6 flex justify-center items-center bg-white rounded-full 
                            ${toastMsg.type == "error"?"text-error-500":"text-success-500"}
                            `}>
                                {toastMsg.type == "error"?
                                <RxCross2 />
                                :
